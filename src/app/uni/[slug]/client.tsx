...
export default function UniClient({ slug }: { slug: string }) {
  ...
  if (!uni) return <div className="max-w-4xl mx-auto px-4 py-20 text-center"><h1 className="text-3xl font-bold mb-4">Не найдено</h1><Link href="/universities" className="text-brand-600 hover:underline">← Назад</Link></div>

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <Link href="/" className="hover:text-brand-600">Главная</Link><span>/</span>
        <Link href="/universities" className="hover:text-brand-600">Университеты</Link><span>/</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3">🏛️ Информация</h3>
          <div className="space-y-2 text-sm">
            {uni.f && <div className="flex justify-between"><span className="text-gray-500">Основан</span><span className="font-medium">{uni.f}</span></div>}
            {uni.d && <div className="flex justify-between"><span className="text-gray-500">Тип</span><span className={uni.d === 'Private' ? 'text-purple-600 font-medium' : 'text-green-600 font-medium'}>{uni.d === 'Private' ? 'Частный' : 'Государственный'}</span></div>}
            {uni.students && <div className="flex justify-between"><span className="text-gray-500">Студентов</span><span className="font-medium">{uni.students}</span></div>}
            {uni.accreditation && <div className="flex justify-between"><span className="text-gray-500">Аккредитация</span><span className="font-medium text-xs">{uni.accreditation}</span></div>}
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      {uni.desc && (
        <div className="mb-8">
          ...
          <p className="text-gray-700">{uni.desc}</p>
        </div>
      )}
      ...
      {uni.history && (
        <div className="mb-8">
          ...
          <p className="text-gray-700 text-sm">{uni.history}</p>
        </div>
      )}
      ...
      {uni.l && uni.l.length > 0 && (
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">{uni.l.map(lang => <span key={lang} className="bg-brand-50 text-brand-700 px-3 py-1.5 rounded-full text-sm">{lang}</span>)}</div>
        </div>
      )}

      {uni.u && uni.u.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-3">📚 Факультеты ({uni.u.length})</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {uni.u.map((fac, i) => (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-4">
                <h3 className="font-semibold text-gray-900 mb-1">{fac.name}</h3>
                {fac.fields?.length > 0 && <div className="flex flex-wrap gap-1 mt-2">{fac.fields.map(f => <span key={f} className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{f}</span>)}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="text-center mt-8">
        <Link href="/universities" className="text-brand-600 hover:underline">← Назад к поиску</Link>
      </div>
    </div>
  )
}