  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t.universities.title}</h1>
          <p className="text-gray-600 text-xs sm:text-sm">{t.universities.subtitle}</p>
        </div>
        {selected.size > 0 && (
          <button onClick={goCompare} disabled={selected.size < 2}
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-xl text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${selected.size >= 2 ? 'bg-brand-600 text-white hover:bg-brand-700 shadow-lg shadow-brand-200' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-3 sm:mb-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder={t.universities.search} value={search}
            ...
            className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2.5 sm:py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-600 text-sm" />
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 mb-3 sm:mb-4">
        <div>
          ...
          <select value={country} onChange={(e) => setCountry(e.target.value)} className="w-full px-2 sm:px-3 py-2 rounded-lg border border-gray-300 bg-white text-xs sm:text-sm">
        ...
        <div>
          ...
          <select value={funding} onChange={(e) => setFunding(e.target.value)} className="w-full px-2 sm:px-3 py-2 rounded-lg border border-gray-300 bg-white text-xs sm:text-sm">
        ...
        <div>
          ...
          <select value={language} onChange={(e) => setLanguage(e.target.value)} className="w-full px-2 sm:px-3 py-2 rounded-lg border border-gray-300 bg-white text-xs sm:text-sm">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4 pb-2 sm:pb-3 border-b border-gray-200 gap-1">
        <span className="text-xs sm:text-sm text-gray-600"><strong>{total.toLocaleString()}</strong> {t.universities.found}</span>
        {totalPages > 1 && <span className="text-xs sm:text-sm text-gray-500">{t.universities.page} {page} {t.universities.of} {totalPages}</span>}
      </div>

      {loading ? (
        <div className="text-center py-12"><div className="animate-spin text-3xl sm:text-4xl mb-4">🔍</div><p className="text-gray-500 text-sm">{t.universities.loading}</p></div>
      ) : items.length === 0 ? (
        <div className="text-center py-16 text-gray-500"><p className="text-3xl sm:text-4xl mb-3">😕</p><p className="text-sm">{t.universities.noResults}</p></div>
      ) : (
        <>
                  ...
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-2 text-xs text-gray-500 mb-1.5 sm:mb-2">
                        ...
                        {u.t && <span className="bg-gray-100 px-1.5 sm:px-2 py-0.5 rounded-full text-xs">📍 {u.t}</span>}
                        {u.d && <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-xs ${u.d === 'Private' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>{u.d === 'Private' ? t.universities.private : t.universities.public}</span>}
                        {u.f && <span className="bg-gray-100 px-1.5 sm:px-2 py-0.5 rounded-full text-xs">{t.universities.foundedLabel} {u.f}</span>}
                        {(u as any).students && <span className="bg-green-50 text-green-700 px-1.5 sm:px-2 py-0.5 rounded-full text-xs">{t.universities.students} {(u as any).students}</span>}
                      </div>
                      {(u as any).desc && <p className="text-xs text-gray-500 mb-1.5 sm:mb-2 line-clamp-2">{(u as any).desc}</p>}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 sm:gap-4 mt-6 sm:mt-8">
              <button onClick={() => goPage(page - 1)} disabled={page <= 1} className="px-3 sm:px-4 py-2 rounded-xl border border-gray-300 text-xs sm:text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-50">{t.universities.prev}</button>
              <div className="flex gap-1">
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  const start = Math.max(1, Math.min(page - 2, totalPages - 4))
                  const p = start + i
                  if (p > totalPages) return null
                  return <button key={p} onClick={() => goPage(p)} className={`w-7 h-7 sm:w-10 sm:h-10 rounded-lg text-xs sm:text-sm font-medium border transition-all ${p === page ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-gray-700 border-gray-300 hover:border-brand-400'}`}>{p}</button>
                })}
              </div>
              <button onClick={() => goPage(page + 1)} disabled={page >= totalPages} className="px-3 sm:px-4 py-2 rounded-xl border border-gray-300 text-xs sm:text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-50">{t.universities.next}</button>
            </div>
          )}
        </>
      )}
