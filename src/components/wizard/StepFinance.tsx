  ...
  return (
    <div className="max-w-2xl mx-auto text-center">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {presets.map((val) => (
          <button key={val} onClick={() => { setBudget(val); setScholarshipOnly(false) }}
            className={`p-4 rounded-xl border-2 font-medium transition-all ${budget === val && !scholarshipOnly ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-200 hover:border-gray-300'}`}>
            {val === 0 ? 'Бесплатно' : `$${val.toLocaleString()}`}
          </button>
        ))}
      </div>
      <button onClick={() => setScholarshipOnly(!scholarshipOnly)}
        className={`px-6 py-3 rounded-xl border-2 font-medium transition-all mb-8 ${scholarshipOnly ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-200 hover:border-gray-300'}`}>
        🎓 Ищу стипендию
      </button>
      ...
      <button onClick={() => onNext(budget, scholarshipOnly)}
