...
const SUBJECTS = [
  'Computer Science', 'Data Science & AI', 'Engineering', 'Business & Management',
  'Medicine', 'Law', 'Psychology', 'Arts & Design',
]
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        {SUBJECTS.map((sub) => (
          <button key={sub} onClick={() => setSubject(sub)}
            className={`p-4 rounded-xl border-2 text-sm transition-all ${subject === sub ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-200 hover:border-gray-300'}`}>
            {sub}
          </button>
        ))}
      </div>
      <button onClick={() => onNext(subject)} disabled={!subject}
